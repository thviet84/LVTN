...
exports.create = async (req, res, next) => {
  try {
    var message = "";
    const oder = await Oder.findOne({ customer: req.body.customer }).populate(
      "customer"
    );
    if (
      req.body.infoBill.pay == undefined ||
      req.body.infoBill.pay == "Thanh toán khi nhận hàng" ||
      req.body.infoBill.pay == "--Lựa chọn--"
    ) {
      req.body.infoBill.pay = "Thanh toán khi nhận hàng";
    }
      ...
      var mailOptions = {
        ...
        html: `   
                        ...
                        <div>
                            ...
                            <h4>THV Laptop Store xin chân thành cảm ơn quý khách. Xin chúc quý khách hàng ngày càng thành công trong công việc, gặp nhiều may mắn và sức khỏe.</h4>
                        </div>
                    `,
      };
...
exports.update = async (req, res, next) => {
  try {
    const currentBill = await Bill.findById(req.params.id).populate("customer");
    var bill;
    var nhanvien;
    if (
      currentBill.status === "Đã giao hàng thành công" ||
      currentBill.status === "Đơn hàng đã bị hủy bỏ"
    ) {
